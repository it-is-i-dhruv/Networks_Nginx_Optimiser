...
print("Nginx Optimisation started.")
...
def events_opt(events, conf):
    print("<--- starting multi accept optimisations --->")
    conf.append(events)
    multi_line = "multi_accept on;\n"
    conf.append(multi_line)
    print("<---- finished multi accept optimisations --->")
    return None

def listen_opt(listen, conf):
    print("<---- starting reuseport (socket sharding) optimisations --->")
    listen_split = listen.strip().split()
    listen_split[1] = '80 reuseport;\n'
    final_line = ' '.join(listen_split)
    conf.append(final_line)
    print("<---- finished reuseport (socket sharding) optimisations --->")
...
print("Nginx Optimisation Completed.")
